...
func setupAuthRoutes(mux *http.ServeMux, deps *Dependencies) {
	...
	mux.Handle("/api/auth/login", sessionMW(http.HandlerFunc(deps.AuthHandler.Login)))
	mux.Handle("/api/auth/logout", sessionMW(http.HandlerFunc(deps.AuthHandler.Logout)))
	mux.Handle("/api/auth/register", sessionMW(http.HandlerFunc(deps.AuthHandler.Register)))
...
func setupPrintRequestRoutes(mux *http.ServeMux, deps *Dependencies) {
	...
	mux.Handle("/api/print-requests", sessionMW(authMW(printRequestHandler)))

	// Print request status updates
	statusHandler := createPrintRequestStatusHandler(deps.PrintRequestHandler)
	mux.Handle("/api/print-requests/status", sessionMW(authMW(statusHandler)))
}
...
func setupUserRoutes(mux *http.ServeMux, deps *Dependencies) {
	...
	mux.Handle("/api/user/print-requests", sessionMW(authMW(userRequestsHandler)))
}
...
func setupAdminRoutes(mux *http.ServeMux, deps *Dependencies) {
	...
	mux.Handle("/api/admin/users", sessionMW(authMW(modMW(adminUsersHandler))))

	// Admin user role updates (admin only)
	adminUserRoleHandler := createAdminUserRoleHandler(deps.AdminHandler)
	mux.Handle("/api/admin/users/role", sessionMW(authMW(adminMW(adminUserRoleHandler))))

	// Admin user status updates (moderator+)
	adminUserStatusHandler := createAdminUserStatusHandler(deps.AdminHandler)
	mux.Handle("/api/admin/users/status", sessionMW(authMW(modMW(adminUserStatusHandler))))

	// Admin stats (moderator+)
	adminStatsHandler := createAdminStatsHandler(deps.AdminHandler)
	mux.Handle("/api/admin/stats", sessionMW(authMW(modMW(adminStatsHandler))))
	...
	mux.Handle("/api/admin/print-requests", sessionMW(authMW(modMW(adminPrintRequestsHandler))))

	// Admin spoolman config (moderator+)
	adminSpoolmanConfigHandler := createAdminSpoolmanConfigHandler(deps.AdminHandler)
	mux.Handle("/api/admin/spoolman-config", sessionMW(authMW(modMW(adminSpoolmanConfigHandler))))
}
...
func setupSpoolmanRoutes(mux *http.ServeMux, deps *Dependencies) {
	...
	mux.Handle("/api/spoolman/spools", sessionMW(authMW(spoolsHandler)))

	spoolHandler := createSpoolHandler(deps.SpoolmanHandler)
	mux.Handle("/api/spoolman/spool", sessionMW(authMW(spoolHandler)))

	materialsHandler := createMaterialsHandler(deps.SpoolmanHandler)
	mux.Handle("/api/spoolman/materials", sessionMW(authMW(materialsHandler)))
}
